...
function toggleTheme() {
    if (theme.value === 'light') {
      ...
      let style: HTMLLinkElement | null = document.getElementById('theme') as HTMLLinkElement;
...
<template>
    <div class="h-20 mb-20 grid grid-cols-3 border-b border-[#C5C5C5]">
        ...
        <div class="flex h-full items-center space-x-16 pr-28 justify-end">
            <label for="theme-toggle" class="md:inline-block hidden w-12 h-6 rounded-full cursor-pointer">
                ...
                <div @click="toggleTheme()" class="main relative w-full h-full rounded-full bg-[#FFD951]"></div>
            </label>
            <img v-if="theme === 'light'" src="../assets/secret_box_logo_black.svg" alt="Secret boxes logo">
            <img v-if="theme === 'dark'" src="../assets/logo.png" class="w-12 h-12" alt="Secret boxes logo">
        </div>
    </div>
</template>
